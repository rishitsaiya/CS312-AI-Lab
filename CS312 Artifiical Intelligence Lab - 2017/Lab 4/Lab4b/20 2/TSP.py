...
class simulated_annealing:
	def __init__(self, number_of_towns, towns, cooling_rate, initial_temperature, closing_temperature, closing_iterations, initial_solution):
		# self.current_solution = solution(nearest_neighbour_solution(towns), towns)
	...
	def get_next_solution(self):
		...
		while (True):
			...
			if probability > random_number:
				# print ("Best Cost: ", end = "")
				# print (self.best_solution.calculate_cost_of_state(), end = "\t Current Cost: ")
				# print (i.calculate_cost_of_state(), end = "\t Current Temp: ")
				# print (self.current_temperature, end = "\t probability: ")
				# print (probability)
...
def nearest_neighbour_solution(towns):
	total = [i for i in range(len(towns))]
	l = [0]
	while len(l) != len(towns):
		last_city = l[-1]
		k = [i for i in total if i not in l]
		distance_set = [(towns[last_city].get_distance_to_town(i), i) for i in k]
		distance_set.sort()
		l.append(distance_set[0][1])
	return l
...
def main():
	...
	ants1 = ant_colony_optimisation(number_of_towns, towns, 100, 1.0, 10.0, 0.5, 1, initial_pheromone, 1)
	final_cost_aco = (ants1.run_ants())
	annealing1 = simulated_annealing(number_of_towns, towns, 0.995, 4, 1.1, 1000, final_cost_aco)
	final_cost_annealing = annealing1.run_sim()
	print (final_cost_annealing)
	file = open("20.txt", "w+")
	file.write(final_cost_annealing.__str__())
