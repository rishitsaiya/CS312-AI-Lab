...
public:
    ...
    virtual int minMaxAlgo(OthelloBoard &board, Turn turn, int depth, Move move);
...
Move MyBot::play(const OthelloBoard &board)
{
    ...
    for (Move nextMove : nextMoves)
    {
        ...
        int heuristicValue = minMaxAlgo(gameBoard, this->turn, depth, nextMove);
...
int MyBot::minMaxAlgo(OthelloBoard &board, Turn turn, int depth, Move move)
{
    ...
    if (this->turn == turn)
    {
        ...
        {
            bestValue = min(bestValue, minMaxAlgo(gameBoard, other(turn), depth - 1, move));
    ...
    else
    {
        for (Move move : moveTree)
        {
            bestValue = max(bestValue, minMaxAlgo(gameBoard, other(turn), depth - 1, move));
